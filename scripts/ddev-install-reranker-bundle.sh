...
env_file=".mxp-reranker-env"
python3 -c 'import shlex, sys
keys = ["RERANKER_ID", "HF_REPO", "REVISION", "MODEL_PATH", "TOKENIZER_PATH", "DEST", "MAX_TOKENS", "EXPECTED_MODEL_SHA256", "EXPECTED_TOKENIZER_SHA256", "EXPECTED_MODEL_SIZE", "EXPECTED_TOKENIZER_SIZE"]
for key, value in zip(keys, sys.argv[1:]):
    print(f"export {key}={shlex.quote(value)}")
' "$RERANKER_ID" "$HF_REPO" "$REVISION" "$MODEL_PATH" "$TOKENIZER_PATH" "$DEST" "$MAX_TOKENS" "$EXPECTED_MODEL_SHA256" "$EXPECTED_TOKENIZER_SHA256" "$EXPECTED_MODEL_SIZE" "$EXPECTED_TOKENIZER_SIZE" > "$env_file"
...
export model_sha tokenizer_sha model_size tokenizer_size
php -r '\''
$manifest = [
