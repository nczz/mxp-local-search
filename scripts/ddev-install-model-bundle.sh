...
env_file=".mxp-model-env"
python3 -c 'import shlex, sys
keys = ["MODEL_ID", "HF_REPO", "REVISION", "MODEL_PATH", "TOKENIZER_PATH", "DEST", "DIMENSIONS", "EXPECTED_MODEL_SHA256", "EXPECTED_TOKENIZER_SHA256", "EXPECTED_MODEL_SIZE", "EXPECTED_TOKENIZER_SIZE"]
for key, value in zip(keys, sys.argv[1:]):
    print(f"export {key}={shlex.quote(value)}")
' "$MODEL_ID" "$HF_REPO" "$REVISION" "$MODEL_PATH" "$TOKENIZER_PATH" "$DEST" "$DIMENSIONS" "$EXPECTED_MODEL_SHA256" "$EXPECTED_TOKENIZER_SHA256" "$EXPECTED_MODEL_SIZE" "$EXPECTED_TOKENIZER_SIZE" > "$env_file"
